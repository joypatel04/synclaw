  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      await ctx.db.patch(existing._id, {
        ...
        keyIvHex: enc.ivHex,
        status: "untested",
        lastAppliedStatus: "pending",
        lastAppliedAt: now,
        lastAppliedError: undefined,
        lastRuntimeValidatedAt: undefined,
        lastRuntimeValidationStatus: undefined,
...
export const listWorkspaceKeyStatus = query({
  ...
  handler: async (ctx, args) => {
    ...
    return rows.map((row) => ({
      ...
      lastAppliedStatus: row.lastAppliedStatus ?? "pending",
      lastAppliedError: row.lastAppliedError ?? null,
      lastRuntimeValidatedAt: row.lastRuntimeValidatedAt ?? null,
      lastRuntimeValidationStatus: row.lastRuntimeValidationStatus ?? null,
  ...
  handler: async (ctx, args) => {
    ...
    for (const row of rows) {
      ...
      await ctx.db.patch(row._id, {
        ...
        lastRuntimeValidationStatus: status,
        updatedAt: now,
        updatedBy: membership.userId,
      });

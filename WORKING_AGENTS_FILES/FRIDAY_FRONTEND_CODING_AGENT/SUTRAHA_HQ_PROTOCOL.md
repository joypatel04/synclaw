...
1.  **Verify Environment:** `mcporter call sutraha-hq.sutraha_get_agent_by_session_key(sessionKey: "<your-session-key>")`
2.  **Send Pulse:** `mcporter call sutraha-hq.sutraha_agent_pulse(status: "active", sessionKey: "<your-session-key>")`

### Stage 2: Task Pickup
1.  **Link Task:** `mcporter call sutraha-hq.sutraha_start_task_session(taskId: "<taskId>", sessionKey: "<your-session-key>")`
2.  **Update Task Status:** `mcporter call sutraha-hq.sutraha_update_task_status(taskId: "<taskId>", status: "in_progress", sessionKey: "<your-session-key>")`

### Stage 3: Execution
*   Perform the work. Use **Memory Tiers** as defined in `MEMORY_PROTOCOL.md`.

### Stage 4: Resolution & Telemetry (Termination)
1.  **Post Result:** `mcporter call sutraha-hq.sutraha_send_message(content: "<detailed summary of work>", taskId: "<taskId>", sessionKey: "<your-session-key>")`
2.  **End Session:** `mcporter call sutraha-hq.sutraha_end_task_session(status: "idle", sessionKey: "<your-session-key>", telemetry: {"totalTokensUsed": <number>, "lastRunCost": <number>}, runSummary: "<short summary>")`
...
mcporter call sutraha-hq.sutraha_send_message \
  taskId="kx73sc..." \

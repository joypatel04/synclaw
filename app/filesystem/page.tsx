...
function FilesystemContent() {
  ...
  const openDirectory = async (path: string) => {
    ...
    try {
      ...
      setCurrentPath(String((result as any)?.path ?? path));
      const nextItems = Array.isArray((result as any)?.items)
        ? ((result as any).items as FilesystemNode[])
        : [];
      setItems(nextItems);
    } catch (e) {
  ...
  const openFile = async (path: string) => {
    ...
    try {
      ...
      setSelectedSize(
        typeof (result as any)?.size === "number" ? (result as any).size : undefined,
      );
      setSelectedMtimeMs(
        typeof (result as any)?.mtimeMs === "number" ? (result as any).mtimeMs : undefined,
      );
      setDirty(false);
  ...
  useEffect(() => {
    ...
    void openDirectory(".");
  }, [filesystemState, items.length]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="mb-6 overflow-hidden rounded-2xl border border-border-default bg-bg-secondary">
        <div className="flex items-center justify-between border-b border-border-default px-4 py-3 sm:px-6">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-lg font-bold text-text-primary sm:text-xl">Filesystem</h1>
          ...
          <div className="flex items-center gap-2 text-[11px]">
            <span className="inline-flex items-center gap-1 rounded-md border border-border-default px-2 py-1 text-text-muted">
              ...
              {hasBridgeConfigured ? "Bridge configured" : "Bridge not configured"}
            </span>
            <span className="inline-flex items-center gap-1 rounded-md border border-border-default px-2 py-1 text-text-muted">
              {canEditFiles ? <PencilLine className="h-3 w-3" /> : <Lock className="h-3 w-3" />}
      ...
      {filesystemState === "disabled" ? (
        <div className="rounded-xl border border-border-default bg-bg-secondary p-6">
          <p className="text-sm text-text-muted">
            ...
            <code className="font-mono text-xs">NEXT_PUBLIC_OPENCLAW_FILES_ENABLED=true</code>.
          </p>
        </div>
      ) : null}
            ...
            <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
              <div className="hidden lg:block">
                <FilesystemTree
                  ...
                  loading={busy}
                  onRefresh={() => void openDirectory(currentPath)}
                  onOpenParent={() => void openDirectory(parentPath(currentPath))}
              ...
              <div className="space-y-3">
                ...
                />
                {error ? <p className="text-xs text-status-blocked">{error}</p> : null}
              </div>
            </div>
          )}

          <div className="rounded-xl border border-border-default bg-bg-secondary p-4 sm:p-6">
            <h3 className="text-sm font-semibold text-text-primary">Local fallback editor</h3>
